...
def render_markdown_with_local_images(md: str):
    ...
    while i < len(parts):
        ...
        if src.startswith("http://") or src.startswith("https://"):
            st.image(src, caption=caption or (alt or None), width=500)
        else:
            img_path = _resolve_image_path(src)
            if img_path.exists():
                st.image(str(img_path), caption=caption or (alt or None), width=500)
            else:
...
# ✅ Show a warning if OpenAI API key is missing
...
with st.sidebar:
    ...
    with st.expander("💰 Cost estimate (OpenAI)"):
        st.markdown(
            """
| What | Model | Cost |
|---|---|---|
| Text (per blog) | gpt-4o-mini | ~$0.02–0.05 |
| Images (×3) | dall-e-3 | ~$0.12 |
